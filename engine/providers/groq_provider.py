...
class GroqProvider(BaseProvider):
    """High-speed Groq inference provider."""

    ENDPOINT = "https://api.groq.com/openai/v1/chat/completions"

    def __init__(self, model_name: Optional[str] = None):
        super().__init__("groq", "Groq Fast Chat")
        self.model_name = model_name or DEFAULT_MODELS["groq_chat"]

    def _get_api_key(self) -> Optional[str]:
        key = get_db().get_setting("groq_api_key")
        if not key:
        ...
        **kwargs
        ...
        if not api_key:
            return ProviderResponse(
                ...
                success=False,
                error_msg="Groq API key is not configured. Please connect it in 'Add AI Models' in the sidebar."
            )
        ...
        if system_prompt:
            ...
            messages.append({
                "role": "system",
                "content": "You are Sage AI, a brilliant, concise, and helpful desktop AI assistant."
            })
        ...
        active_model = kwargs.get("model") or get_db().get_setting("groq_model") or self.model_name
        payload = {
            "model": active_model,
            "messages": messages,
        ...
        try:
            if on_chunk:
                payload["stream"] = True
                resp = requests.post(self.ENDPOINT, headers=headers, json=payload, stream=True, timeout=45)
                if resp.status_code != 200:
                    ...
                    return ProviderResponse(
                        ...
                        provider_id=self.provider_id,
                        success=False,
                        error_msg=f"Groq API Error ({resp.status_code}): {error_info}"
                    )
            ...
            return ProviderResponse(
                ...
                success=False,
                latency_ms=latency,
                error_msg=f"Groq connection error: {str(e)}"
            )
